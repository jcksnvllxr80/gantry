var Gantt = function() {
    ...
    var h = {
        ...
        to_string(t, e = !1) {
            ...
            const s = this.get_date_values(t).map(((t, e) => (1 === e && (t += 1), d(t + "", 6 === e ? 3 : 2, "0")))),
                i = `${s[0]}-${s[1]}-${s[2]}`,
                n = `${s[3]}:${s[4]}:${s[5]}.${s[6]}`;
            return i + (e ? " " + n : "")
        },
        format(t, e = "YYYY-MM-DD HH:mm:ss.SSS", s = "en") {
            ...
            let a = e;
            const o = [];
            return Object.keys(n).sort(((t, e) => e.length - t.length)).forEach((t => {
    ...
    }, p.closest = (t, e) => e ? e.matches(t) ? e : p.closest(t, e.parentNode) : null, p.attr = (t, e, s) => {
        if (!s && "string" == typeof e) return t.getAttribute(e);
        if ("object" != typeof e) t.setAttribute(e, s);
        else
            for (let s in e) p.attr(t, s, e[s])
    };
    class u {
        constructor(t, e) {
        ...
        show_popup() {
            ...
            const t = h.format(this.task._start, "MMM D", this.gantt.options.language) + " - " + h.format(h.add(this.task._end, -1, "second"), "MMM D", this.gantt.options.language);
    ...
    class f {
        ...
        setup_tasks(t) {
            this.tasks = t.map(((t, e) => {
                ...
                if (h.get_date_values(t._end).slice(3).every((t => 0 === t)) && (t._end = h.add(t._end, 24, "hour")), t.start && t.end || (t.invalid = !0), "string" == typeof t.dependencies || !t.dependencies) {
                    let e = [];
        ...
        setup_date_values() {
            this.dates = [];
            let t = null;
            for (; null === t || t < this.gantt_end;) t = t ? this.view_is(w.YEAR) ? h.add(t, 1, "year") : this.view_is(w.MONTH) ? h.add(t, 1, "month") : h.add(t, this.options.step, "hour") : h.clone(this.gantt_start), this.dates.push(t)
        }
        bind_events() {
        ...
        setup_layers() {
            ...
            for (let e of t) this.layers[e] = _("g", {
                class: e,
                append_to: this.$svg
            })
        }
        ...
        make_grid_rows() {
            ...
            for (let a of this.tasks) _("rect", {
                x: 0,
                y: n,
                width: s,
                height: i,
        ...
        make_grid_ticks() {
            ...
            for (let i of this.dates) {
                let n = "tick";
                this.view_is(w.DAY) && 1 === i.getDate() && (n += " thick"), this.view_is(w.WEEK) && i.getDate() >= 1 && i.getDate() < 8 && (n += " thick"), this.view_is(w.MONTH) && (i.getMonth() + 1) % 3 == 0 && (n += " thick"), _("path", {
                    d: `M ${t} ${e} v ${s}`,
                    class: n,
                    append_to: this.layers.grid
                }), this.view_is(w.MONTH) ? t += h.get_days_in_month(i) * this.options.column_width / 30 : t += this.options.column_width
            }
        }
        ...
        set_scroll_position() {
            ...
            if (!t) return;
            const e = h.diff(this.get_oldest_starting_date(), this.gantt_start, "hour") / this.options.step * this.options.column_width - this.options.column_width;
            t.scrollLeft = e
        }
        bind_grid_click() {
            p.on(this.$svg, this.options.popup_trigger, ".grid-row, .grid-header", (() => {
        ...
        bind_bar_events() {
            let t = !1,
                ...
                a = null,
                o = [];
            this.bar_being_dragged = null, p.on(this.$svg, "mousedown", ".bar-wrapper, .handle", ((r, h) => {
